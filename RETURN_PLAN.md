...
- **돋보기** (FR-M3 · FR2-M1) — 사진 아래 우측 버튼으로 켜고, 켜진 상태에서만 포인터를 따라
  렌즈가 확대한다. 포인터가 나가면 렌즈는 사라지고(FR2-M2), 키보드 조작 중에는 유지된다.
...
- **커뮤니티 기여 섹션** (FR-O2) — 기관 기록 *옆에* 두고 절대 섞지 않는다.
  ...
  전체 개수를 함께 밝힌다
- **문서·녹음 목록** (FR2-D1) — 사진이 아닌 공개 자산도 기록에 표시한다. 캐러셀과 성격이
  달라 별도 목록으로 두고, 종류·크기·출처를 함께 밝힌다
