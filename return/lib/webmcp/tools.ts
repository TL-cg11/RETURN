...
export const communityTools: ToolSpec[] = [
  ...
  { name: 'submit_evidence', description: 'Submit a photograph, document, or oral-history asset for curator review. Submission does not change the public record.', readOnly: false, properties: { object_id: { type: 'string' }, title: { type: 'string' }, description: { type: 'string' }, source: { type: 'string' }, consent: { type: 'string', description: 'private, research_only, public_anonymous, or public_attributed.' } }, required: ['object_id', 'title', 'description', 'consent'] },
...
export const curatorTools: ToolSpec[] = [
  ...
  { name: 'list_objects', description: 'List collection objects with provenance status and new-contribution counts.', readOnly: true, properties: { status: { type: 'string' } } },
  ...
  { name: 'compare_evidence', description: 'Compare a submitted source against the verified record for dates, custody, consent, and contradictions.', readOnly: true, untrusted: true, properties: { evidence_ids: { type: 'array', items: { type: 'string' } } }, required: ['evidence_ids'] },
