...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const body = await request.json() as { action?: string; draft?: string; editReason?: string };
  ...
  if (approval.snapshot_hash !== immutableHash && approval.snapshot_hash !== legacyHash) {
    return Response.json({ outcome: 'denied', policy: 'approval_snapshot_mismatch', reason: 'The approval snapshot changed after review was requested.', recovery: 'Create a new approval request for the current draft.' }, { status: 409 });
  }
  ...
  await db.prepare('UPDATE approvals SET status=?, resolution=?, verdict=?, edited_body=?, edit_reason=?, resolved_at=? WHERE museum_id=? AND id=?')
    .bind(resolution, resolution, body.action, edited ? draft : null, edited ? String(body.editReason ?? '') : null, Date.now(), museumId, id).run();
  await recordActivity(museumId, 'Mina, Curator',
    ...
    `Moonbird Mask · revision ${approval.object_version + 1}`, {
      actorRole: 'curator_ui', actorType: 'human', tool: approval.tool, target: approval.object_id,
      risk: 'HIGH', policyDecision: body.action === 'approved' ? 'applied' : 'denied', result: resolution,
    });

  return Response.json({ id, status: resolution, resolution, edited, persisted: true });
}
